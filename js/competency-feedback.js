...
function isSectionComplete(s){
  if(s==='A'){var checks=document.querySelectorAll('#sectionA input[type="checkbox"]');var checked=Array.from(checks).filter(function(c){return c.checked});return checked.length>=3}
  if(s==='B'){return ['softB1','softB2','softB3','softB4','softB5','softB6'].every(function(n){return document.querySelector('input[name="'+n+'"]:checked')!==null})}
  if(s==='C'){var prep=document.querySelector('input[name="overallPrep"]:checked')!==null;var el=document.getElementById('sectionC');var tas=el.querySelectorAll('textarea[data-required="true"]');var allTa=Array.from(tas).every(function(t){return t.value.trim()!==''});return prep&&allTa}
  return false;
}
...
function submitForm(){
  var names={A:'Technical Skills',B:'Soft Skills',C:'Overall Assessment'};var inc=['A','B','C'].filter(function(s){return !isSectionComplete(s)});
  if(inc.length>0){var m=inc.map(function(s){return 'Section '+s+' ('+names[s]+')'}).join('\n  • ');alert('Please complete all required fields:\n  • '+m);scrollToSection('section'+inc[0])}
  else{showToast('Competency feedback submitted successfully!')}
}
